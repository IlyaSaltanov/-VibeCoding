...
// Итоговый вариант для первой дз
// let csvData = [];
...
document.getElementById('csvFile').addEventListener('change', function (e) {
    ...
    reader.onload = function (event) {
        ...
        csvData = parseCSV(text);
        console.log("Данные загружены:", csvData.length, "строк"); // Лог
        populateYearSelect(); // Обновляем выпадающий список
    };
    reader.onerror = function () {
        alert("Ошибка чтения файла!");
    };
...
function parseCSV(text) {
    ...
    const headers = lines[0].split(',');
    return lines.slice(1).map(line => {
        const values = line.split(/,(?=(?:[^"]*"[^"]*")*[^"]*$)/); // Обработка кавычек
        return headers.reduce((obj, header, i) => {
            obj[header.trim()] = values[i]?.trim() || '';
            return obj;
        }, {});
...
function processData() {
    ...
    // Агрегация данных по городам и штатам
    const cityStateData = {};
    let totalRevenue = 0;
    ...
    csvData.forEach(row => {
        ...
        if (year === selectedYear) {
            ...
            if (city && state) {
                ...
                if (!cityStateData[key]) {
                    cityStateData[key] = { key, revenue: 0, orders: 0 };
                }
                cityStateData[key].revenue += sales;
                cityStateData[key].orders += 1; // +1 заказ на каждую строку
            }
        }
    });

    // Сортировка и выбор топ-10
    const sortedCities = Object.values(cityStateData)
        .sort((a, b) => b.revenue - a.revenue)
        .slice(0, 10);
    ...
    titleRow.innerHTML = `
        <td colspan="4">
            <strong>Топ 10 городов по выручке за ${selectedYear} год</strong><br>
            <em>Общая выручка: $${totalRevenue.toFixed(2)}</em>
        </td>
    `;
    thead.appendChild(titleRow);
    ...
    // Шапка таблицы
    const headerRow = document.createElement('tr');
    ['№', 'Город (Штат)', 'Заказы', 'Выручка'].forEach(text => {
        const th = document.createElement('th');
        th.textContent = text;
        headerRow.appendChild(th);
    });
